...
The strand artifact filter detects sequencing artifact in which the evidence for the alt allele consists entirely of forward reads alone or reverse reads alone. We must detect this while taking into account the fact that at some loci, such as near the end of an exome target, \emph{all} reads are biased towards one direction, and thus the bias towards one read direction among the alt is not suspect.

Let $z$ be a discrete latent random variable that represents the artifact state of a suspected variant, where $z \in \{ \text{Artifact Forward}, \text{Artifact Reverse}, \text{No Artifact} \}$. At each locus, the distributions over forward alt reads $x^+$ and reverse alt reads $x^-$, conditional on the artifact state $z$ as well as the allele fraction $f$ and artifact allele fraction $\epsilon$, are binomial random variables . Artifactual allele fraction $\epsilon$ is the probability that we mistakenly read a ref allele as alt due to a strand artifact error. We give the flat prior over $f$ and a beta prior over $\epsilon$. We define the conditional probability of $x^+$ as

\begin{equation}
P(x^+ | f, \epsilon, z) =
\begin{cases}
\text{Bin} (x^+ | n^+, f + \epsilon(1-f)) & z = \text{Artifact Forward}\\
\text{Bin} (x^+ | n^+, f) 			& z = \text{Artifact Reverse or No Artifact}\\
\end{cases}
\end{equation}

The conditional distributions of $x^-$ is defined analogously.

In order to assess whether a variant should be filtered, we compute the posterior probabilities of the artifact states $z$ given the observed alt read counts $X = \{ x^+, x^- \}$.  We compute the likelihood function $P(X | z)$ by marginalizing the latent variables $f$ and $\epsilon$ out of the joint distribution $P(X, f, \epsilon | z)$, and we do this for each of the three possible values of $z$ separately. First consider the case where $z = \text{Artifact Forward}$, which we denote by the shorthand $z^+$ below. The likelihood $P(X | z^+ )$ is

\begin{align}
P(X | z^+ )  &= \iint_{f, \epsilon}  P(X, f, \epsilon | z^+) \,df\,d\epsilon \nonumber \\
			  &= \iint_{f, \epsilon}  P(f) p(\epsilon) P(X | z^+, f, \epsilon) \,df\,d\epsilon \nonumber \\
			  &= \iint_{f, \epsilon}  P(f) p(\epsilon) p(x^+ | z^+, f, \epsilon) p(x^- | z^+, f, \epsilon) \,df\,d\epsilon \nonumber \\
			  &= \iint_{f, \epsilon}  P(\epsilon) p(x^+ | z^+, f, \epsilon) p(x^- | z^+, f, \epsilon) \,df\,d\epsilon \label{watershed} \\
			  &= \iint_{f, \epsilon}  \mathrm{Beta}(\epsilon|\alpha, \beta) \mathrm{Bin}(x^+ | f + \epsilon(1-f), n^+) \mathrm{Bin}(x^- | f, n^-) \,df\,d\epsilon
\end{align}

where $\alpha$ and $\beta$ are fixed hyperparameters to the beta prior over $\epsilon$. By Bayes Rule the posterior probability of strand bias for forward reads is therefore

\begin{align}
P(z^+ | X) & \propto P(z^+) P(X | z^+) = \pi_+ P(X | z^+) 
\end{align}

where $\pi_+ = P(z^+)$ is a fixed prior probability of strand artifact. The probability of reverse strand artifact, $P(z^- | X)$, may be derived analogously. The derivation of the the posterior probability of no artifact is identical up to (\ref{watershed}). The integral then simplifies because the conditional probabilities of $x^+$ and $x^-$ do not depend on $\epsilon$. Using a shorthand $z_o$ for $z= \text{No Artifact}$, we get

\begin{align}
P(X | z_o)  &= \iint_{f, \epsilon}  p(\epsilon) P(x^+ | z_o, f, \epsilon) P(x^- | z_o, f, \epsilon) \,df\,d\epsilon \nonumber \\
			  &= \int_{f}  P(x^+ | z_o, f) P(x^- | z_o, f) \,df \int_{\epsilon}  P(\epsilon) d\epsilon \\
...
where $\pi_o = P(z = \text{No Artifact})$ is the fixed prior probability of no artifact. We normalize the posteriors and filter the variant if the posterior probability of the forward or the reverse strand artifact exceeds the pre-determined threshold.
