...
import static org.broadinstitute.hellbender.tools.walkers.readorientation.CollectDataForReadOrientationFilter.ALL_KMERS;
...
public class LearnHyperparameters extends CommandLineProgram {
    ...
    public Object doWork(){
        int numRows = 1_000_000; // TODO: come up with a better heuristic
        ...
        List<Hyperparameters> hyperparameterEstimates = new ArrayList<>((int) Math.pow(REGULAR_BASES.size(), REFERENCE_CONTEXT_SIZE)/2);

        for (final String refContext : ALL_KMERS){
            Optional<Histogram<Integer>> histogram = histograms.stream().filter(h -> h.getValueLabel().equals(refContext)).findFirst();
            ...
            final LearnHyperparametersEngine engine = new LearnHyperparametersEngine(histogram.get(), altDesignMatrixForContext,
                    converagenceThreshold, maxEMIterations, logger);
